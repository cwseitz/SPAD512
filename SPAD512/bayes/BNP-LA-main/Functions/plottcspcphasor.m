function [G,S] = plottcspcphasor(decay,ref,ref_tau,freq,delta_t) %decay
...
ph_cor = -atan2(S_ref,G_ref)+ph_ref;     %calculate data phasor
Gn=0;
Sn=0;
area=0;
...
Gdec = Gn./area;
Sdec = Sn./area;
G = abs((Gdec.*cos(ph_cor) - Sdec.*sin(ph_cor))./M_cor); 
S = abs((Gdec.*sin(ph_cor) + Sdec.*cos(ph_cor))./M_cor);     %Plot phasor point and universal circle
